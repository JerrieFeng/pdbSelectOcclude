...
There are 5 functions in `pdbSelectOcclude`. All which require *bio3d*, *r3dmol*, and *dplyr*. If we wish to interact with the 3D protein models, then Shiny will also need to be installed.
...
* The **selAASlider** A function that allow users to interact with any AA with a slider 
through Shiny App. Use the slider to select which AA to view. 
Example:
```r
pdbFile <- bio3d::read.pdb(bio3d::get.pdb("1bm8"))
...
* The **selChainSlider** A function that allow users to interact with any chain with a slider viewed on Shiny App.
Example:
```r
pdbFile <- bio3d::read.pdb(bio3d::get.pdb("1SI4"))
